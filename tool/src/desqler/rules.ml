    ...
    let accessed_common_table : S.st -> S.st -> string option = 
      fun stmt1 -> fun stmt2 ->
        match (stmt1,stmt2) with
          |(S.SELECT ((t_name_s,_,_,_),_,_,_) , S.UPDATE ((t_name_u,_,_,_),_,_,_)) -> 
            if t_name_s = t_name_u
            then Some t_name_s
            else None
...
(*auxiliary function to find the common accessed table*)
    let accessed_common_table : S.st -> S.st -> string option = 
      fun stmt1 -> fun stmt2 ->
        match (stmt1,stmt2) with
          |( S.UPDATE ((t_name_u1,_,_,_),_,_,_) , S.UPDATE ((t_name_u2,_,_,_),_,_,_) ) -> 
            if t_name_u1 = t_name_u2
            then Some t_name_u1
            else None
          |_ -> failwith "ERROR accessed_common_table: sql operation case not handled yet"
